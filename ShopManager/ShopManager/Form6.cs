...
{
    public partial class Form6 : frmBase
    {
        public Form6()
        {
            InitializeComponent();
        ...
        private void getContents()
        {
            ...
            try
            {
                ...
                {                    
                    SqlParameter[] param = {};
                    ds = biz.ExecuteDataset(param, "USP_TB_PRODUCT_SELECT");
        ...
        private void btnDelete_Click(object sender, EventArgs e)
        {
            ...
            string colProductID = dataGridView1.CurrentRow.Cells["colProductID"].Value.ToString();


            if (colProductID == "")
            {
                MessageBox.Show("삭제할 상품이 선택되지 않았습니다.", this.Text, MessageBoxButtons.OK, MessageBoxIcon.Error);
                return;
            }
            ...
            try
            {
                using (DB_Biz biz = new DB_Biz())
                {
                    DataSet ds = new DataSet();
                    SqlParameter[] param = {
                      new SqlParameter("@PRODUCT_ID", colProductID),
                    };

                    int rtn = biz.ExecuteNonQuery(param, "USP_TB_PRODUCT_DELETE");
        ...
        private void btnSave_Click(object sender, EventArgs e)
        {
            ...
            for (int i = 0; i < dataGridView1.Rows.Count; i++)
            {
                ...
                if (hc.Value != null)
                {
                    ...
                    {
                        string colProductID = dataGridView1.Rows[i].Cells["colProductID"].Value.ToString();
                        string colProductName = dataGridView1.Rows[i].Cells["colProductName"].Value.ToString();
                        ...
                        if (string.IsNullOrEmpty(colProductName) ||
                        ...
                        try
                        {
                            using (DB_Biz biz = new DB_Biz())
                            {
                                DataSet ds = new DataSet();
                                SqlParameter[] param = {
                                  new SqlParameter("@PRODUCT_ID", colProductID == "" ? "0" : colProductID),
                                  new SqlParameter("@PRODUCT_NAME", colProductName),
                                  new SqlParameter("@PRICE", Price),
                                  new SqlParameter("@USE_YN", USE_YN)
                                };

                                ds = biz.ExecuteDataset(param, "USP_TB_PRODUCT_SAVE");
                                ...
                                if (ds.Tables.Count > 0)
                                {
                                    if (ds.Tables[0].Rows.Count > 0)
                                    {
                                        if (colProductID == "")
                                        {
                                            dataGridView1.Rows[i].Cells["colProductID"].Value = ds.Tables[0].Rows[0]["PRODUCT_ID"].ToString();
                                        }
